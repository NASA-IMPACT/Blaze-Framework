...
PID_PATH_NAME="consul/service-pid"
...
case $1 in
    linux)
        if [ ! -f $PID_PATH_NAME ]; then
            ...
            unzip -o consul_1.7.1_linux_amd64.zip -d consul
            rm consul_1.7.1_linux_amd64.zip
            nohup consul/consul agent -config-file={{ mft_deployment_directory }}/consul-server.json > $LOG_FILE 2>&1 &
            echo $! > $PID_PATH_NAME
            echo "Consul started"
        else
